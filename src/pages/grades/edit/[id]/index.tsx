...
import { useRoqClient, useGradeFindFirst } from 'lib/roq';
...
function GradeEditPage() {
  ...
  const { data, error, isLoading, mutate } = useGradeFindFirst(queryParams, {}, { disabled: !id });
  const [formError, setFormError] = useState(null);

  const handleSubmit = async (values: GradeInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await roqClient.grade.update({
        data: values as RoqTypes.grade,
        where: {
          id,
        },
      });
