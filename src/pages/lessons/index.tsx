...
import { useRoqClient, useLessonFindManyWithCount } from 'lib/roq';
...
export function LessonListPage(props: LessonListPageProps) {
  ...
  const queryParams = useMemo(
    () =>
      convertQueryToPrismaUtil(
        {
          relations: ['assignment', 'company.count'],
  ...
  const { data, error, isLoading, mutate } = useLessonFindManyWithCount({
  ...
  const handleDelete = async (id: string) => {
    setDeleteError(null);
    try {
      await roqClient.lesson.delete({ where: { id } });
      await mutate();
    } catch (error) {
  ...
  const columns: ColumnType[] = [
    ...
    hasAccess('company', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          ...
          header: 'Company',
          accessorKey: 'company',
          cell: ({ row: { original: record } }: any) => record?._count?.company,
        }
      : null,
