...
import { useRoqClient, useLessonFindFirst } from 'lib/roq';
...
function LessonEditPage() {
  ...
  const { data, error, isLoading, mutate } = useLessonFindFirst(queryParams, {}, { disabled: !id });
  const [formError, setFormError] = useState(null);

  const handleSubmit = async (values: LessonInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await roqClient.lesson.update({
        data: values as RoqTypes.lesson,
        where: {
          id,
        },
      });
